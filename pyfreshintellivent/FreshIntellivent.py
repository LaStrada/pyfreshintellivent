...
class Sky(object):
    ...
    def authenticate(self, auth):
        ...
        try:
            self._conn.getCharacteristics(
                uuid=characteristics.AUTH)[0].write(
    ...
    def _writeCharacteristic(self, uuid, value):
        ...
        try:
            self._conn.getCharacteristics(uuid=uuid)[0].write(
                value, withResponse=True)
    ...
    def _log(self, level, message):
        if self._debug:
            print("[Fresh Intellivent Sky] [{}] {}").format(
                level, message)

    def getHumidity(self):
        value = unpack(
            '<BBH',
            self._readCharacterisitc(uuid=characteristics.HUMIDITY))
    ...
    def getLightVOC(self):
        value = unpack(
            '<4B',
            self._readCharacterisitc(uuid=characteristics.LIGHT_VOC))
    ...
    def getConstantSpeed(self):
        value = unpack(
            '<BH',
            self._readCharacterisitc(uuid=characteristics.CONSTANT_SPEED))
    ...
    def setConstantSpeed(self, constantSpeedEnabled, constantSpeedRPM):
        value = pack(
            ...
            h.validatedRPM(constantSpeedRPM))
    ...
    def getTimer(self):
        value = unpack(
            '<3BH',
            self._readCharacterisitc(uuid=characteristics.TIMER))
    ...
    def getAiring(self):
        value = unpack(
            '<3BH',
            self._readCharacterisitc(uuid=characteristics.AIRING))
    ...
    def getPause(self):
        value = unpack(
            '<2B',
            self._readCharacterisitc(uuid=characteristics.PAUSE))
    ...
    def setPause(self, pauseEnabled, pauseMinutes):
        value = pack(
            '<2B',
            bool(pauseEnabled),
            h.validatedMinutes(pauseMinutes))
    ...
    def getSensorData(self):
        value = unpack(
            '<2B5HBH',
            self._readCharacterisitc(uuid=characteristics.DEVICE_STATUS))
